...
def main():
    ...
    with tab2:
        ...
        chart_tab1, chart_tab2, chart_tab3, chart_tab4 = st.tabs([
            "📊 Performance", "🥧 Operations", "⏱️ Lifecycle", "🌐 Topology"
        ])
        ...
        with chart_tab1:
            ...
            if perf_chart:
                ...
                st.info("No performance data available yet. Send some messages to see analytics!")
        ...
        with chart_tab3:
            st.markdown("#### Entry Lifecycle Timeline")
            lifecycle_chart = create_entry_lifecycle_chart(analytics_data)
            if lifecycle_chart:
                st.plotly_chart(lifecycle_chart, use_container_width=True)
            else:
                st.info("No entries in lifecycle tracking.")
        
        with chart_tab4:
            st.markdown("#### Network Topology")
            topology_chart = create_network_topology_chart(dht_status)
            if topology_chart:
                st.plotly_chart(topology_chart, use_container_width=True)
            else:
                st.info("No active entries to visualize topology.")
        ...
        if dht_status['active_entries'] > 0:
            ...
            for key, entry in dht_status['entries'].items():
                ...
                entries_data.append({
                    ...
                    'TTL Remaining': f"{int(ttl_remaining//60)}m {int(ttl_remaining%60)}s",
                    'Size (bytes)': entry.get('size_bytes', 0),
                    'Age (s)': int(time.time() - entry['created']),
                    'Status': '🟢 Active' if ttl_remaining > 60 else '🟡 Expiring Soon'
                })
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "TTL Remaining": st.column_config.TextColumn("⏳ TTL", width="small"),
                    "Size (bytes)": st.column_config.NumberColumn("📦 Size", width="small"),
                    "Age (s)": st.column_config.NumberColumn("🕐 Age", width="small"),
        ...
        with col_filter1:
            log_count = st.slider("Show last N logs", 5, 50, 20)
        with col_filter2:
            operation_filter = st.selectbox("Filter by operation", ["All", "PUT", "GET", "DELETE", "RESET"])
        with col_filter3:
            if st.button("🔄 Refresh Logs"):
                st.rerun()
        
        logs = st.session_state.dht.get_recent_logs(log_count)
        ...
        if logs:
            # Create a DataFrame for better log visualization
            log_data = []
            for log in reversed(logs):
                ...
                if operation == 'PUT':
                    ...
                    icon = "🔄"
                    color = "#e91e63"
                    status = f"CLEARED {log.get('entries_cleared', 0)} entries"
                else:
                    icon = "ℹ️"
                    color = "#888"
                    status = "UNKNOWN"
                ...
                log_data.append({
                    'Time': log['timestamp'],
                    'Operation': f"{icon} {operation}",
                    'Key': log.get('key', 'N/A')[:12] + '...' if 'key' in log else 'N/A',
                    'Status': status,
                    'Details': f"Size: {log.get('size_bytes', 'N/A')} bytes" if 'size_bytes' in log else 
                              f"Age: {log.get('age_seconds', 'N/A')}s" if 'age_seconds' in log else 'N/A'
                })
            
            # Display logs as a styled table
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Details": st.column_config.TextColumn("ℹ️ Details", width="small")
                }
            )
            ...
            with col_stat1:
                put_count = len([log for log in logs if log['operation'] == 'PUT'])
                st.metric("PUT Operations", put_count)
            
            with col_stat2:
                get_count = len([log for log in logs if log['operation'] == 'GET'])
                st.metric("GET Operations", get_count)
            
            with col_stat3:
                success_count = len([log for log in logs if log.get('status') == 'SUCCESS'])
                st.metric("Successful GETs", success_count)
            
            with col_stat4:
                error_count = len([log for log in logs if log.get('status') in ['EXPIRED', 'NOT_FOUND']])
                st.metric("Failed GETs", error_count)
        else:

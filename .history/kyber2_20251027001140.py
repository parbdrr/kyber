...
class DistributedHashTable:
    ...
    def __init__(self, reset_interval: int = 10):  # 10 seconds default
    ...
    def put(self, key: str, value: any, ttl: int = None) -> bool:
        ...
        with self.lock:
            ...
            self.table[key] = {
                ...
                'original_ttl': ttl if ttl else self.reset_interval
            }
...
def init_session_state():
    ...
    if 'dht' not in st.session_state:
        st.session_state.dht = DistributedHashTable(reset_interval=10)  # 10 seconds
...
def main():
    ...
    with tab1:
        if not st.session_state.current_user:
            ...
            current_ttl = st.session_state.get('dht_ttl', 10)
            st.info(f"⏱️ **DHT TTL:** {current_ttl} seconds (entries will expire after this time)")
            ...
            if st.button("🔒 ENCRYPT & SEND VIA DHT", type="primary", use_container_width=True):
                if recipient and message_text:
                    ...
                    with st.spinner("🌐 Splitting and routing through DHT..."):
                        ...
                        ttl_value = st.session_state.get('dht_ttl', 10)
                        st.session_state.dht.put(dht_key, part2, ttl=ttl_value)
    ...
    with tab2:
        ...
        # Show current TTL setting for new entries
        current_ttl = st.session_state.get('dht_ttl', 10)
        st.info(f"🎛️ **Current TTL Setting:** {current_ttl} seconds (applies to NEW entries only)")
            ...
            for key, entry in dht_status['entries'].items():
                ...
                with st.expander(f"🔑 {key[:12]}... - {entry.get('size_bytes', 0)} bytes"):
                    ...
                    with col2:
                        ...
                        # Show original TTL used when entry was created
                        original_ttl = entry.get('original_ttl', 'Unknown')
                        st.caption(f"Original TTL: {original_ttl}s")
                    ...
                    max_ttl = st.session_state.get('dht_ttl', 10)
                    progress = ttl_remaining / max_ttl
        ...
        # TTL Control Slider
        st.markdown("### ⏱️ TTL CONTROL")
        col1, col2, col3 = st.columns([2, 1, 1])
        
        with col1:
            new_ttl = st.slider(
                "DHT Entry TTL (seconds)", 
                min_value=5, 
                max_value=300, 
                value=st.session_state.get('dht_ttl', 10),
                step=5,
                help="How long entries stay in the DHT before expiring",
                key="ttl_slider"
            )
            # Update TTL immediately when slider changes
            if new_ttl != st.session_state.get('dht_ttl', 10):
                st.session_state.dht_ttl = new_ttl
                st.success(f"✓ TTL updated to {new_ttl} seconds")
        
        with col2:
            st.metric("Current TTL", f"{new_ttl}s")
        
        with col3:
            if st.button("🔄 Reset to 10s", use_container_width=True):
                st.session_state.dht_ttl = 10
                st.success("✓ TTL reset to 10 seconds")
                st.rerun()
